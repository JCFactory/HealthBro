    ...
    EnvironmentDeserializer.Parse = function (arrayBuffer) {
        ...
        for (var i = 0; i < magicBytes.length; i++) {
            if (dataView.getUint8(pos++) !== magicBytes[i]) {
                tools_1.Tools.Error('Not a Spectre environment map');
            }
        }
        var version = dataView.getUint16(pos, littleEndian);
        pos += 2;
        if (version !== 1) {
            tools_1.Tools.Warn('Unsupported Spectre environment map version "' + version + '"');
        }
        ...
        switch (descriptor.irradiance.type) {
            case 'irradiance_sh_coefficients_9':
                ...
                break;
            default:
                tools_1.Tools.Error('Unhandled MapType descriptor.irradiance.type (' + descriptor.irradiance.type + ')');
        }
        //specular
        switch (descriptor.specular.type) {
            case 'cubemap_faces':
                var specularDescriptor = descriptor.specular;
                var specularTexture = environment.specularTexture = new texture_1.TextureCube(6408 /* RGBA */, 5121 /* UNSIGNED_BYTE */);
                    ...
                    for (var i = 0; i < 6; i++) {
                        ...
                        switch (imageType) {
                            case 'png':
                                ...
                                break;
                            default:
                                tools_1.Tools.Error('Unhandled ImageType descriptor.specular.imageType (' + imageType + ')');
                        }
                    }
                }
                break;
            default:
                tools_1.Tools.Error('Unhandled MapType descriptor.specular.type (' + descriptor.specular.type + ')');
        }
        return environment;
    };
    /**
...
exports.EnvironmentDeserializer = EnvironmentDeserializer;
