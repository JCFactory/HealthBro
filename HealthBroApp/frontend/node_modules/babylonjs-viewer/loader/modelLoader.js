...
var glTFFileLoader_1 = require("babylonjs-loaders/glTF/glTFFileLoader");
var tools_1 = require("babylonjs/Misc/tools");
var sceneLoader_1 = require("babylonjs/Loading/sceneLoader");
...
var ModelLoader = /** @class */ (function () {
    ...
    ModelLoader.prototype.addPlugin = function (plugin) {
        ...
        if (typeof plugin === 'string') {
            var loadedPlugin = plugins_1.getLoaderPluginByName(plugin);
            if (loadedPlugin) {
    ...
    ModelLoader.prototype.load = function (modelConfiguration) {
        var _this = this;
        var model = new viewerModel_1.ViewerModel(this._observablesManager, modelConfiguration, this._configurationContainer);
        ...
        if (modelConfiguration.file) {
            ...
            filename = tools_1.Tools.GetFilename(modelConfiguration.url) || modelConfiguration.url;
            this._baseUrl = modelConfiguration.root || tools_1.Tools.GetFolderPath(modelConfiguration.url);
        }
        if (!filename || !this._baseUrl) {
            model.state = viewerModel_1.ModelState.ERROR;
            tools_1.Tools.Error("No URL provided");
            return model;
        }
        var plugin = modelConfiguration.loader;
        var scene = model.rootMesh.getScene();
        model.loader = sceneLoader_1.SceneLoader.ImportMesh(undefined, this._baseUrl, filename, scene, function (meshes, particleSystems, skeletons, animationGroups) {
            meshes.forEach(function (mesh) {
                tags_1.Tags.AddTagsTo(mesh, "viewerMesh");
                model.addMesh(mesh);
            });
            ...
            model.state = viewerModel_1.ModelState.ERROR;
            tools_1.Tools.Error("Load Error: There was an error loading the model. " + m);
        ...
        if (model.loader.name === "gltf") {
            ...
            gltfLoader_1.animationStartMode = glTFFileLoader_1.GLTFLoaderAnimationStartMode.NONE;
            gltfLoader_1.compileMaterials = true;
            if (!modelConfiguration.file) {
                gltfLoader_1.rewriteRootURL = function (rootURL, responseURL) {
                    return modelConfiguration.root || tools_1.Tools.GetFolderPath(responseURL || modelConfiguration.url || '');
                };
            }
    ...
    ModelLoader.prototype.cancelLoad = function (model) {
        ...
        if (loader && loader.name === "gltf") {
            ...
            gltfLoader.dispose();
            model.state = viewerModel_1.ModelState.CANCELED;
        }
        else {
            tools_1.Tools.Warn("This type of loader cannot cancel the request");
        }
    };
    /**
